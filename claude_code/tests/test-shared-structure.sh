...
#   bash claude_code/tests/test-shared-structure.sh
...
SETUP_SCRIPT="$CLAUDE_CODE_DIR/claude-setup.sh"
PASS=0
FAIL=0
...
# ══════════════════════════════════════════════════════════════
# 9. Regression — Claude-specific files still in place
# ══════════════════════════════════════════════════════════════

echo ""
echo "=== regression: Claude-specific files untouched ==="

# CLAUDE.md still exists
assert_file_exists "CLAUDE.md exists" "$CLAUDE_CODE_DIR/.claude/CLAUDE.md"

# settings.json still exists
assert_file_exists "settings.json exists" "$CLAUDE_CODE_DIR/.claude/settings.json"

# Agents still present (not moved to shared/)
...
for f in "${AGENT_FILES[@]}"; do
  assert_file_exists "agent $f exists" "$CLAUDE_CODE_DIR/.claude/agents/$f"
done

# Hooks still present (not moved to shared/)
HOOK_FILES=(verify-on-stop.sh verify-after-edit.sh auto-format.sh protect-files.sh reinject-context.sh notify.sh)
for f in "${HOOK_FILES[@]}"; do
  assert_file_exists "hook $f exists" "$CLAUDE_CODE_DIR/.claude/hooks/$f"
done

# Claude-specific docs still in claude_code/docs/
...
for f in "${CLAUDE_DOCS[@]}"; do
  assert_file_exists "claude doc $f exists" "$CLAUDE_CODE_DIR/docs/$f"
done

# Commands still present
assert_dir_exists "commands/ exists" "$CLAUDE_CODE_DIR/.claude/commands"

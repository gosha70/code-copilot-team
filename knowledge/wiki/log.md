...
- 2026-05-20 — created `pre-pr-close-keyword-audit` (playbook) + `scripts/pre-pr-check.sh`: consolidates three recurring PR-mechanics failures (commit-message close-keyword scan with code-span stripping; inline `gh pr create --title`; `--body-file` readability verified in the same shell); driven by PR #53 accidentally closing epic #34 via plain-text `Closes #34` in the TB1.1 commit message body. Cross-links memory `feedback_close_keyword_audit_pre_pr`.

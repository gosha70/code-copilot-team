...
try {
  ...
  const rowsText = '{"sessions": [{"id": 35, "project_path": "/Users/x/dev/repo/proj", "errors": 71}], "count": 1}';
  ex = av.applyEvent(ex, { event: "result", n: 1, summary: "5 sessions", chars: 9000, truncated: true, error: null, session_ids: [35, 41, 7, 8, 9], result_text: rowsText, seconds: 0.02 });
  ...
  else if (ex.steps[0].resultText !== rowsText || !ex.steps[0].resultText.includes('"errors": 71')) fail("the step keeps the rows the judge was given, not only a count");
  else console.log("  ok  a finished lookup says what it returned, that it was cut, its arguments, and keeps the actual rows");
  ex = av.applyEvent(ex, { event: "step", n: 2, tool: "session_turns", args: { session_id: 35 }, why: "read", seconds: 3 });
  ex = av.applyEvent(ex, { event: "result", n: 2, summary: "error: session 35 not found", chars: 40, truncated: false, error: "session 35 not found", session_ids: [], result_text: '{"error": "session 35 not found"}', seconds: 0.01 });
  ...
  // The model cites #35 and #41 (both returned by a lookup) and #999 (never seen).
  ex = av.applyEvent(ex, { event: "answer", markdown: "Session #35 had 71 errors; see also #41 and #999. Not #319 (an issue).", sessions: [35, 41], unverified: [999], evidence: [35, 41, 7, 8, 9], seconds: 6, steps: 2 });
  ...
  else if (JSON.stringify(ex.unverified) !== JSON.stringify([999])) fail("an id no lookup returned is kept apart as unverified");
  else console.log("  ok  verified citations come first, then the ones lookups returned; an unseen id is unverified, not linked");
  ...
  else if (linked.includes("[#999]")) fail("an unverified id was linked as a session");
  else console.log("  ok  #id in the answer links to the session only for ids a lookup returned");
  ...
  const rvRun = (o) => ({
    ...
    disposition: { reason: null, detail: null, phase: null }, live: false,
  ...
  const rvLive = rvRun({ key: "2-2", status: "building", outcome: null, live: true });
  ...
  // FR-8: polling while any run is live.
  if (rv.shouldPoll(null) || rv.shouldPoll(rvPayload([rvRun(), rvTerminated]))) fail("no live run → no polling");
  else if (!rv.shouldPoll(rvPayload([rvRun(), rvLive]))) fail("a live run → polling");
  else if (!/^live — last state write /.test(rv.liveLine(rvLive, new Date("2026-09-09T16:32:00Z"))) || rv.liveLine(rvRun()) !== "" || !/not concluded — status building/.test(rv.liveLine(rvRun({ status: "building", outcome: null, live: false })))) fail(`live line: ${rv.liveLine(rvLive)}`);
  else console.log("  ok  polls while a run is live and stops when none is; a run that stopped writing without concluding says so");

# Spec: runtime conformance evaluator, increment C2 (#242)
...
- **FR-4 — the frozen conformance contract.** The preflight initialiser
  ...
  drift disposes): the evaluator provider id, the app-launch contract
  (FR-6), `timeout_sec`, and the CRITERIA SET — every
  `runtime_conformance` criterion with its owning `FR-N` and
  `statement_sha`. Gates MUST read only the frozen copy; editing
  `verification.yaml` or `automation.json` after initialisation moves
  nothing. The preflight-result schema's closed `contract` object gains an
  optional closed `conformance` sub-object (absent when the run has no
...
- **FR-5 — the evaluator contract (fresh context, running app).** The
  ...
  - `CCT_CONFORMANCE_RESULT` — path the evaluator MUST write verdicts to:
    `{ "criteria": [ {fr, verdict: "pass"|"fail", evidence} … ] }`,
  ...
  A result file that is missing, unparseable, schema-invalid, or that
  omits any frozen criterion MUST be treated as evaluator failure — never
  as a pass (fail closed; a criterion cannot pass by absence).
...
- **FR-6 — driver-owned app lifecycle.** The driver, not the evaluator,
  ...
  "stop_timeout_sec" }`. Start before the evaluator, prove readiness (a
  ready probe that fails within its bound is an evaluator-gate failure,
  fail closed), and stop the WHOLE process group afterwards with
...
- **FR-7 — the landing verifier ledger (handoff item 1).** `landed`
  ...
  of the frozen artifact — `kind: deterministic` entries are green iff
  the final `test.command` gate passed (their resolution was admission's
  screen; their execution is the test gate); `runtime_conformance`
  entries carry the evaluator's per-criterion verdict and evidence. Any
  non-green or unresolved entry MUST dispose: park (attended) /
  `terminated_policy` (unattended), naming the FR(s) and verdict(s). The
  summary artifact repeats the table.
...
- **SC-3** The frozen conformance contract pins evaluator, app contract,
  and criteria; editing `verification.yaml` or the config after
  initialisation changes nothing at the gate (asserted), and disk drift
  disposes exactly as C1's tamper rule does.
- **SC-4** The evaluator receives the criteria file and the RUNNING app
  (readiness proven); its verdicts land in `verification-results.json`
  per FR; a failing criterion parks/terminates naming the FR; a missing
  or incomplete result file fails closed (never pass-by-absence).
- **SC-5** App lifecycle: ready-probe timeout fails the gate; after the
  gate no descendant of the app process group survives (asserted with a
  marker child, the cp_run_bounded pattern).
...
- **SC-8** All suites green; every SC regression verified to fail against

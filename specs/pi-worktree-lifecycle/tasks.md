...
| 3b | | `pi-code worktree run <workerId> --branch <b> [prov flags] -- <pi args>` — provision, export `CCT_WORKER_*`, re-exec `pi-code --project <worktree> <pi args>` so pi starts with `cwd = worktree` (atomic provision+spawn happy path). The re-exec is an intentional child handoff, so it clears `CCT_PI_CODE_ACTIVE` for that one exec (child re-sets it) — a controller session (`CCT_PI_CODE_ACTIVE=1`) can launch workers without tripping the recursion guard (review P2). Forwarded args re-enter the launcher's parser, so the worktree/enforcement are kept immutable: a forwarded `--project`/`--no-cct` is rejected **pre-provision** (no orphan) AND the child launch is pinned by `CCT_LOCKED_PROJECT_PATH` (refuses cwd ≠ worktree, or `--no-cct`) as the authoritative backstop (review P1). (FR-1) | `bin/pi-code` | one command provisions + launches pi inside the worktree, incl. under an active controller; forwarded `--project`/`--no-cct` cannot escape or disable enforcement (both blocked, no orphan); verified with a pi shim |

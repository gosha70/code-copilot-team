...
Note: refreshed after the user's independent upstream-doc review: the
not-a-filesystem-boundary statement is now prominent (P1) with the
write-preventive / edit-post-execution / CI-authoritative split, the
same-file concurrency attribution limitation is named (P2a), and the
honesty canary is CI-mandatory with a pinned pi fixture (P2b). Scope
unchanged; the user independently confirmed the round-1 API corrections.

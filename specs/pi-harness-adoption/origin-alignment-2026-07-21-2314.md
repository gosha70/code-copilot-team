...
Supersedes `origin-alignment-2026-07-21-2303.md`, which went stale when
`spec.md` was amended. Re-assessed after that amendment.
...
What changed since the superseded record:
  - `spec.md` gained a scope note on the Acceptance Criteria section
    recording that the umbrella DoD is not the merge gate for PR #107,
    which delivers a rescoped slice.
  - `tasks.md` gained a delivery-status section, seven checked tasks,
    and per-task partial/not-started annotations.
  - `.github/workflows/pi-tests.yml` now runs the three Pi suites in CI.
...
Mismatches:
  ...
  - The DoD scope note narrows what PR #107 claims to have delivered.
    It does not narrow what the spec sets out to build, so it is not a
    divergence from the origin — the umbrella criteria are unchanged
    and remain the target for the feature branch.
...
outside this gate, now documented in `tasks.md` and the spec's
Acceptance Criteria scope note.

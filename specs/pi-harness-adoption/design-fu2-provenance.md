...
| **security classification** | capability `security_level`s (enforcing/critical/…) | available |
...
security:     enforcing: N · critical: M · advisory: K
...
| element | status |
|---|---|
| version / source / scope / trust / capability + security summary | **exposed** (composed from real surfaces) |
| dependency status (runtime: none) | **exposed** (stated; full graph in the SBOM) |
| checksum / full SBOM at runtime | **absent → null + pointer** (release artifacts) — honest, not fabricated |
...
1. **Version source** — `CCT_VERSION` env from the launcher (+ `package.json`
   ...
   file install needed).
2. **Absent-at-runtime fields** — `checksum` and the full SBOM/dep-graph are
   reported **`null`/absent with a pointer** to the release artifacts, not
   fabricated. Confirm.
3. **Composition** — `provenance` aggregates version/source + capability &
   security summary + trust/scope, read-only, `--json` + text; it does not
   duplicate `features`/`doctor`, it composes them. Confirm.

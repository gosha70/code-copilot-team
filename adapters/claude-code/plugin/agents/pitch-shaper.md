...
1. **Read context.** Read `${CLAUDE_PLUGIN_ROOT}/templates/sdd/pitch-template.md` for the output format. Skim `specs/pitches/*/pitch.md` to learn from prior pitches in this repo.
...
4. **Read skills.** From `${CLAUDE_PLUGIN_ROOT}/skills/`: `clarification-protocol/SKILL.md`, `spec-workflow/SKILL.md` (if present).
...
9. **Write `specs/pitches/<id>/pitch.md`** from `${CLAUDE_PLUGIN_ROOT}/templates/sdd/pitch-template.md`. Frontmatter must be complete:
...
10. **Validate.** Run `bash ${CLAUDE_PLUGIN_ROOT}/templates/sdd/validate-pitch.sh --pitch-id <id>`. If it fails, fix the frontmatter and re-validate. **Refuse to declare done until it passes.**
...
A populated `specs/pitches/<NNNN-slug>/pitch.md` that passes `${CLAUDE_PLUGIN_ROOT}/templates/sdd/validate-pitch.sh`. Print the path and the validator's "[PASS]" line as confirmation.

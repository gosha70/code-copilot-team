...
1. **Read skills.** At the start, read these from `${CLAUDE_PLUGIN_ROOT}/skills/`:
...
If the `memkernel` MCP server is configured, read `${CLAUDE_PLUGIN_ROOT}/skills/memkernel-memory/SKILL.md` and use it to recall build context at the start of review and retain the final review summary when it would help future sessions.

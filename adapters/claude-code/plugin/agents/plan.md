...
2. **Read skills.** At the start, read these from `${CLAUDE_PLUGIN_ROOT}/skills/`:
...
If the `memkernel` MCP server is configured, read `${CLAUDE_PLUGIN_ROOT}/skills/memkernel-memory/SKILL.md` and use it to recall prior context at the start of planning and retain the approved plan summary when it is worth preserving.

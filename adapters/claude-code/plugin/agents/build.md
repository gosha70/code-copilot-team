...
3. **Read skills.** At the start, read from `${CLAUDE_PLUGIN_ROOT}/skills/`:
...
If the `memkernel` MCP server is configured, read `${CLAUDE_PLUGIN_ROOT}/skills/memkernel-memory/SKILL.md` and use it to recall the approved plan context at the start of implementation and retain key build decisions or checkpoints when they matter.

...
- **Read `${CLAUDE_PLUGIN_ROOT}/skills/token-efficiency/SKILL.md`** at the start for context management guidelines.
...
If the `memkernel` MCP server is configured, read `${CLAUDE_PLUGIN_ROOT}/skills/memkernel-memory/SKILL.md` and use it to recall prior findings or decisions before starting a new research pass.

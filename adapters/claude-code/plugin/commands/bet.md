...
- All required frontmatter fields populated (run `bash ${CLAUDE_PLUGIN_ROOT}/templates/sdd/validate-pitch.sh --pitch-id <pitch-id>` first; refuse on validator failure).
...
Run `bash ${CLAUDE_PLUGIN_ROOT}/templates/sdd/validate-pitch.sh --pitch-id <pitch-id>`. **Refuse if it fails** — restore the previous frontmatter and tell the user.

...
Run `bash ${CLAUDE_PLUGIN_ROOT}/templates/sdd/validate-pitch.sh --pitch-id <pitch-id>`. Refuse on validator failure.
...
Write `specs/pitches/<pitch-id>/hill.json` matching the schema in `${CLAUDE_PLUGIN_ROOT}/templates/sdd/hill-chart.json`:
...
`bash ${CLAUDE_PLUGIN_ROOT}/templates/sdd/validate-pitch.sh --pitch-id <pitch-id>`. Refuse if it fails — restore prior state.

---
name: auto-build-loop
description: "Autonomous build driver after SDD spec approval: phase-scoped headless build sessions, driver-owned commits, cross-provider review gates, milestone pauses, and fail-closed escalation — under explicit opt-in autonomy profiles."
---
...
implemented. A fourth profile, `unattended` (#191, increment A of #190),
exists as policy only: it **fails closed at config load** until #190
increment B ships admission control, must be declared in
`automation.json` itself (`schema_version: 2`, explicit `caps.*`,
validated by `scripts/validate-automation-config.sh`), and can never be
requested via `--profile`/`CCT_AUTOBUILD_PROFILE` override. See
"Terminal outcomes" below for how its breakers dispatch.
...
**Cost metering (FR-7: a $100 cap means $100).** Review rounds now debit
the same `caps.cost_usd` as build/fix sessions: the runner emits
per-invocation cost (`invocation_cost_usd` per round, cumulative `cost`
block in `loop-summary.json`); a reviewer with no cost channel (free-text
CLI providers) is debited as a conservative estimate

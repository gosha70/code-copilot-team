...
Only `advisory` is implemented today. `pr` (#71) and `merge` (later) are
config slots; the driver rejects them until their increments land.

## Invocation

```bash
scripts/auto-build-loop.sh <feature-id> \
    [--profile advisory] [--config <path>] \
...
`--force` code path; commits refused on `master`/`main`; sessions run with

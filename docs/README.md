...
For language models there is an **experimental** [`llms.txt`](../llms.txt):
this same index in the [llms.txt](https://llmstxt.org/) shape, generated by
`scripts/generate-llms-txt.sh` and served at the site's base. It is a proposal,
not a standard, and a project site lives under a path, so a crawler that looks
only at the domain root will not find it.
...
- [Codespace tour — the harness in a browser, nothing to install](codespace-tour.md) — A ten-minute guided tour of Code Copilot Team in a GitHub Codespace: the harness is already installed when the terminal opens, and every step is a command you run and an output you can check

...
class TeamConfig:
    ...
    ``aliases`` maps a recorded ``developer_id`` to a display name; ids
    sharing a name are one person, folded into one row at READ time
    (api.team.team_status). The store is never rewritten."""
...
def _load_team(data: Mapping[str, Any], env) -> TeamConfig:
    ...
    def aliases() -> Mapping[str, str]:
        """team.aliases from the data file, env entries on top (per id).
        A malformed entry refuses rather than being dropped: a silently
        ignored alias puts one person back on the Team tab as several
        developers, which is the bug this setting exists to fix."""
        key = f"{C.CFG_TEAM}.{C.CFG_TEAM_ALIASES}"
        found: dict[str, str] = {}
        block_value = block.get(C.CFG_TEAM_ALIASES) or {}
        if not isinstance(block_value, Mapping):
            raise ValueError(f"{key} must be a mapping of developer id to display name, got {block_value!r}")
        for dev, name in block_value.items():
            if not isinstance(dev, str) or not isinstance(name, str) or not dev.strip() or not name.strip():
                raise ValueError(f"{key} entry {dev!r}: {name!r} must be a non-empty developer id and display name")
            found[dev.strip()] = name.strip()
        ...
        for entry in (raw or "").split(","):
            ...
            if not sep or not dev.strip() or not name.strip():
                raise ValueError(f"{key}: {entry.strip()!r} is not an alias — write id=Name, entries separated by commas")
            found[dev.strip()] = name.strip()
        return found

...
for skill_dir in "$SKILLS"/*/; do
  ...
  case "$name" in coding-standards|copilot-conventions|copyright-headers|origin-confirmation|safety|wiki-first-query) continue ;; esac
...
# Verify 24 .mdc files (all skills)
MDC_COUNT=$(ls "$CURSOR_RULES"/*.mdc 2>/dev/null | wc -l | tr -d ' ')
assert "exactly 24 .mdc files ($MDC_COUNT)" "[[ $MDC_COUNT -eq 24 ]]"
...
assert "exactly 18 instruction files ($ON_DEMAND_COUNT)" "[[ $ON_DEMAND_COUNT -eq 18 ]]"
...
assert "cursor installed 24 .mdc files ($INSTALLED_MDC)" "[[ $INSTALLED_MDC -eq 24 ]]"
rm -rf "$CURSOR_TMP"
...
assert "gh-copilot installed 18 instruction files ($INSTALLED_INSTR)" "[[ $INSTALLED_INSTR -eq 18 ]]"
rm -rf "$GH_TMP"
